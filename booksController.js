...
const booksController = {
  ...
  async delete(request, response, next) {
    ...
    response
      .status(202)
      .json({ message: "Your wish is my command" })
  },
  
  async update() {
    const { author, title } = request.body;
    const { id } = request.params
    const {rows} = await pool.query('UPDATE books SET author = $1, title = $2 WHERE id = $3 RETURNING * ',
      [author, title, id]
    )

    response.status(201).json({ message: "Book was updated to the db...", book: rows[0] });
  }
};
